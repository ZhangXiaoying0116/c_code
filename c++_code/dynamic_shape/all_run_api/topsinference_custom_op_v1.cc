...
TopsInferenceCustomOp::Compute(const OrtApi *api, OrtKernelContext *context,
  ...
  if (!(dynamic_infos.empty())) {
    ...
    std::for_each(
        tops_dynamic_shape.begin(), tops_dynamic_shape.end(), [](auto &p) {
          std::cout << "p{" << p.first << "}\n";
  ...
  if (parser_ && (!engine_ || tensor_shape_changed)) {
    ...
    // const std::string enable_graph_builder_env =
    // env_instance.GetEnvironmentVar(
    //     onnxruntime::topsinference_env_vars::kEnableGraphBuilder);
    // if (!enable_graph_builder_env.empty()) {
    //   enable_graph_builder = (std::stoi(enable_graph_builder_env) == 0 ?
    //   false : true);
    // }
  ...
  if (has_dynamic_shape) {
    ...
    for (int32_t i = 0; i < num_outputs; ++i) {
      ...
      for (int32_t j = 0; j < max_shape.nbDims; ++j) {
        output_shape.push_back(max_shape.dimension[j]); // sample_num*H*W*C
      }
