...
// PlannerImpl::GetElementSize, Given a tensor-type, return the size of an
// element of the tensor.
int32_t GetElementSize(const auto &tensor_type) {
  const auto &type_proto =
      ONNX_NAMESPACE::Utils::DataTypeUtils::ToTypeProto(tensor_type);
...
TopsInferenceCustomOp::TopsInferenceCustomOp(
  ...
  /*
    TODO : Get the type of output, by getting it from the fused_node or
    engine(engine_->getOutputDataType(i))
    Is it possible that fused_node or engine get different types of output
  */
  const auto &output_defs = fused_node->OutputDefs();
  for (int32_t output_arg_def_index = 0, end = output_defs.size();
       output_arg_def_index < end; ++output_arg_def_index) {
    const auto &node_output = output_defs[output_arg_def_index];
    const std::string *output_type_ = node_output->Type();
    int32_t type_size = GetElementSize(output_type_);
    outputs_elementsize.push_back(type_size);
  }
...
TopsInferenceCustomOp::Compute(const OrtApi *api, OrtKernelContext *context,
  ...
  */
  ...
  std::shared_ptr<IAllocator> cpu_allocator = std::make_shared<CPUAllocator>();
  ...
  for (int i = 0; i < num_outputs; ++i) {
    ...
    /*
        New a buffer to store the output results, not bound to output OrtValues.
        For dynamic shapes, you need to new the largest output buffer.
    */
    int64_t output_buffersize = outputs_elementsize[i]; // todo for type
    LOGS(*logger_, VERBOSE) << "Output " << i << "'s shape: ";
    std::for_each(output_shape.begin(), output_shape.end(),
                  [&output_buffersize, logger = GetLogger() ](int64_t n) {
                    output_buffersize *= n;
                    LOGS(*logger, VERBOSE) << "- dim value: " << n;
                  });
    void *output_data = cpu_allocator->Alloc(output_buffersize);
  ...
  /*
    TODO:
    Create the output ortvalue and bind a buffer to it
  */
  for (int i = 0; i < num_outputs; ++i) {
    TopsInference::Dims ouput_shape = output_tensor_list[i]->getDims();
    std::vector<int64_t> ort_shape;
    int64_t output_buffersize = outputs_elementsize[i];
    for (int32_t index = 0; index < ouput_shape.nbDims; ++index) {
      ort_shape.push_back(ouput_shape.dimension[index]);
      output_buffersize *= ouput_shape.dimension[index];
    }
    OrtValue *output_tensor = ort.KernelContext_GetOutput(
        context, 0, ort_shape.data(), ort_shape.size());
    void *output_data = ort.GetTensorMutableData<void>(output_tensor);
    memcpy(output_data, outputs[i], output_buffersize);
    cpu_allocator->Free(outputs[i]);
  }
